...
LRESULT CALLBACK d3d_window::winproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam){
		...
		for( uint32_t i=0;i<application::object_array.m_count;i++){ application::object_array[i]->msgproc(msg,wParam,lParam); }
	...
	case WM_KEYDOWN:{
		...
		if(wParam == VK_SHIFT){
			_application->addflags(application_shiftdown);
		}
		break;
					}
	case WM_KEYUP:{
		if(wParam == VK_SHIFT){
			_application->removeflags(application_shiftdown);
		}
		break;
					}
