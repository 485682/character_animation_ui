...
#define application_title  "character animation ui"
...
struct _utility{
	...
	static void string_insert(const char * in,_string * string,uint32_t start,uint32_t end){
		...
		if(!string  ){ return; }
		bool insert_end = (start==0)&&(end==0); 

		uint32_t in_length  = (!in)   ? 0 : strlen(in);
		uint32_t end_length =  end==0 ? 0 : (string->m_count-end);

		uint32_t all_length = insert_end? (string->m_count+in_length+1) : (start+in_length+end_length+1);

		char * string_buffer = new char[all_length]; 
		application_zero(string_buffer,all_length);

		if(insert_end){
			for(uint32_t i=0; i<string->m_count; i++) { string_buffer[i] = string->m_data[i];         }
			for(uint32_t i=0; i<in_length;       i++) { string_buffer[string->m_count+i] = in[i];               }
		}else{
