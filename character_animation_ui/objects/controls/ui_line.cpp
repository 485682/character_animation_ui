...
bool ui_line::genhighlightbuffer(){
	...
	application_throw_hr(_api_manager->m_d3ddevice->CreateVertexBuffer(6 * sizeof(_vec3),D3DUSAGE_WRITEONLY,0,D3DPOOL_MANAGED,&m_highlight_buffer,0));
...
bool ui_line::update(){
	...
	if(m_string.m_count-m_scroll>0){
		...
		application_throw_hr(_fx->Begin(NULL, 0));
		application_throw_hr(_fx->BeginPass(0));
		application_throw_hr(_api_manager->m_d3ddevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0,m_count*2) );
		application_throw_hr(_fx->EndPass());
		application_throw_hr(_fx->End());
	}
...
void ui_line::msgproc(UINT msg, WPARAM wParam, LPARAM lParam){
	...
	{
	case WM_LBUTTONDOWN :{
		if(intersection_test()){ _application->m_ui->m_current_control = m_id; }
						 }
