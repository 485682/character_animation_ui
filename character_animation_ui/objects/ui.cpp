...
bool ui::init(){
	...
	if(!readfont("courier.bmp") ){ return false; }
	...
	* this loop iterates and sets  the color of the font to ARGB(0xFF,0xFF,0xFF,0xFF)
...
bool ui::addcontrol(ui_control * control){
	...
	application::object_array.pushback((application_object*)control);

	return true;
}

bool ui::readfont(const char * filename){
...
_string ui::getclipboard(){
	...
	if (!IsClipboardFormatAvailable(CF_TEXT)) { application_error("getclipboard"); return result; } 
	if (!OpenClipboard(_window->m_hwnd))  { application_error("getclipboard"); return result; }

	HGLOBAL global = GetClipboardData(CF_TEXT); 
	if (global != NULL) { 
		char* string = (char*)GlobalLock(global); 
		if (string != NULL) { result = string; } 
		else{ application_error("Globallock"); }

		if(!GlobalUnlock(global)){ application_error("GlobalUnlock"); }
	}else { application_error("GetClipboardData"); }

	if(!CloseClipboard()){ application_throw("CloseClipboard"); } 
	return result;
}

bool ui::setclipboard(const _string & text){

	if(text.m_count==0){ application_throw("empty string");}

	// Open the clipboard, and empty it. 
	if(!OpenClipboard(_window->m_hwnd)){ application_throw("hwnd"); } 
	if(!EmptyClipboard()) { application_throw("EmptyClipboard"); }

	// Allocate a global memory object for the text. 
	HGLOBAL hglbCopy = GlobalAlloc(GMEM_MOVEABLE,(text.m_count+1)* sizeof(char)); 
	if (hglbCopy == NULL)  { CloseClipboard(); application_throw("hglbCopy"); } 

	// Lock the handle and copy the text to the buffer. 
	char* buffer = (char*)GlobalLock(hglbCopy); 
	application_zero( buffer,  (text.m_count+1)* sizeof(char) );
	memcpy(buffer, text.m_data, text.m_count* sizeof(char) ); 

	if(GlobalUnlock(hglbCopy)!=NO_ERROR){ application_throw("GlobalUnlock"); }
	// Place the handle on the clipboard. 
	if(SetClipboardData(CF_TEXT, hglbCopy) == NULL) { application_throw("SetClipboardData"); } 
	// Close the clipboard. 
	if(!CloseClipboard()){ application_throw("CloseClipboard"); } 
